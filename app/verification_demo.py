...
async def demo_interceptors():
    ...
    async def test_call():
        ...
        request = ServiceRequest("user-service", "get_user", args=(1,), context=service_context)
        ...
        return await pipeline.execute(interceptor_context, target)
...
async def demo_async_processing():
    ...
    # 并发调用
    tasks = [user_service.get_user(i) for i in range(1, 11)]
    results = await asyncio.gather(*tasks)

    end_time = time.time()
    duration = end_time - start_time

    print(f"   处理了 {len(results)} 个用户")
    print(f"   耗时: {duration:.3f}s")
    print(f"   平均每个用户: {duration/len(results):.4f}s")
...
async def demo_service_integration():
    ...
    print(f"   订单用户: {order['user']['name']}")
    print(f"   订单金额: {order['amount']}")

    print("\n2. 用户订单查询测试:")
    user_orders = await order_service.get_user_orders(1)
    print(f"   用户ID: 1")
    print(f"   订单数量: {len(user_orders)}")
    print(f"   总金额: {sum(order['amount'] for order in user_orders)}")


async def main():
    """主函数"""
    print("🚀 Python 逻辑微服务框架功能验证")
    print("=" * 50)
